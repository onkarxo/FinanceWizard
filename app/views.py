...
def index(request):
    ...
    data = yf.download(
        ...
        # passes the ticker
        tickers=['AAPL', 'AMZN', 'MSFT', 'META', 'NVDA', 'JPM', 'GOOGL', 'SPOT','TSLA'],
    ...
    fig_left = go.Figure()
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['AAPL']['Adj Close'], name="APPLE")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['AMZN']['Adj Close'], name="AMAZON")
            )
    
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['TSLA']['Adj Close'], name="TESLA")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['SPOT']['Adj Close'], name="SPOTIFY")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['JPM']['Adj Close'], name="J.P.MORGAN")
            )
    fig_left.add_trace(
                go.Scatter(x=data['Date'], y=data['GOOGL']['Adj Close'], name="GOOGLE")
            )
...
def news(request):
    ...
    #api_key = "caac246617124f05b98619e21bedbede"
    api_key = "81ab31125789471097c9e5c8c0b1440e"
    # The URL for the News API
    #news_url = f"https://newsapi.org/v2/everything?q=stock+market&apiKey={api_key}"
    news_url = f"https://newsapi.org/v2/everything?q=trading&apiKey={api_key}&pageSize=15"
    ...
    try:
        ...
        articles = []  # Handle any errors gracefully
...
def predict(request, ticker_value, number_of_days):
    ...
    rangeselector=dict(
        buttons=list([
            dict(count=15, label="5m", step="minute", stepmode="backward"),
            dict(count=45, label="10m", step="minute", stepmode="backward"),
            dict(count=1, label="15m", step="hour", stepmode="todate"),
            dict(count=3, label="1h", step="hour", stepmode="backward"),
            dict(step="all")
        ]),
            font=dict(color='red')
        )
    ...
    try:
        df_ml = yf.download(tickers=ticker_value, period='max', interval='1d')
    except:
     return render(request, 'API_Down.html', {})
    ...
    forecast_prediction = model.predict(X_forecast)  # Use test data for prediction
    forecast_predict = model.predict(X_test)
    forecast = forecast_prediction.flatten().tolist()


        # Calculate Mean Squared Error
    confidence = mean_squared_error(y_test, forecast_predict)
    
    r2 = r2_score(y_test, forecast_predict)
    ...
    # Add the predicted data trace to the figure
    pred_fig.add_trace(go.Scatter(x=pred_df['Date'], y=pred_df['Prediction'],   line=dict(color='yellow'), marker=dict(color='red'), name='Predicted Data'))
    ...
    # Create a Plotly figure for the LSTM predictions with monthly intervals
    import plotly.graph_objs as go
    

# Assuming you have calculated forecast_predict and y_test as mentioned in your code

# Create a Plotly figure
    fig_acc = go.Figure()

    # Add actual values from the test set
    fig_acc.add_trace(go.Scatter(x=pred_dict['Date'], y=y_test, mode='lines', name='Actual'))

    # Add predicted values
    fig_acc.add_trace(go.Scatter(x=pred_dict['Date'], y=forecast_predict.flatten(), mode='lines', name='Predicted'))

    # Update the layout
    fig_acc.update_layout(title='Predicted vs Actual Values of {}'.format(ticker_value),
                    xaxis_title='Date',
    ...
    plot_div_acc = plot(fig_acc, auto_open=False, output_type='div')

# ... (Previous code for model training and data preparation)

    # Create a Plotly figure for the LSTM predictions with monthly intervals
    '''fig_lstm = go.Figure()
    fig_lstm.add_trace(go.Scatter(x=df_ml.index.to_period('M').strftime('%Y-%m'), y=y_test, name='Actual Test Data', mode='lines', line=dict(color='blue')))
    fig_lstm.add_trace(go.Scatter(x=df_ml.index.to_period('M').strftime('%Y-%m'), y=forecast_predict.flatten(), name='Predicted Test Data', mode='lines', line=dict(color='red')))
    fig_lstm.add_trace(go.Scatter(x=df_ml.index.to_period('M').strftime('%Y-%m'), y=y_train, name='Actual Train Data', mode='lines', line=dict(color='green')))
    fig_lstm.update_layout(title='Stock Price Prediction with LSTM',
                        xaxis_title='Date',
                        yaxis_title='Stock Price',
                        showlegend=True,
                        ...
                        plot_bgcolor="#14151b",
                        font_color="white")
    plot_div_lstm = plot(fig_lstm, auto_open=False, output_type='div')'''
    ...
    return render(request, "result.html", context={ 'plot_div': plot_div, 
                                                    ...
                                                    'plot_div_acc':plot_div_acc,
                                                    '''plot_div_lstm:plot_div_lstm'''
                                                    'Symbol':Symbol,
                                                    'Name':Name,
                                                    'Last_Sale':Last_Sale,
...
def visualize(request, ticker_value):
    try:
        # ticker_value = request.POST.get('ticker')
        ticker_value = ticker_value.upper()
        df = yf.download(tickers = ticker_value, period='1d', interval='1m')
        df1 = yf.download(tickers = ticker_value, period='5y', interval='1mo')
        df2 = yf.download(tickers = ticker_value, period='max', interval='1d')
        df3 = yf.download(tickers = ticker_value, period='1y', interval='1d')
        print("Downloaded ticker = {} successfully".format(ticker_value))
    except:
        return render(request, 'API_Down.html', {})
    ...
    rangeselector=dict(
        buttons=list([
            dict(count=20, label="1m", step="minute", stepmode="todate"),
            dict(count=1, label="10m", step="hour", stepmode="todate"),
            dict(count=180, label="30m", step="minute", stepmode="todate"),
    ...
    # Calculate Upper and Lower Bands based on SMA and standard deviation
    df2['Upper Band'] = df2['SMA'] + (df2['stddev'] * 2)
    df2['Lower Band'] = df2['SMA'] - (df2['stddev'] * 2)

    # Create a Plotly figure for the Bollinger Bands chart
    fig_bb = go.Figure()

    # Plotting the Closing Price
    fig_bb.add_trace(go.Scatter(x=df2.index, y=df2['Close'], name='Closing Price'))

    # Plotting SMA, Upper Band, and Lower Band
    fig_bb.add_trace(go.Scatter(x=df2.index, y=df2['SMA'], name='SMA', line=dict(color='orange')))
    fig_bb.add_trace(go.Scatter(x=df2.index, y=df2['Upper Band'], name='Upper Band', line=dict(color='red')))
    fig_bb.add_trace(go.Scatter(x=df2.index, y=df2['Lower Band'], name='Lower Band', line=dict(color='green')))
    ...
    fig_bb.update_layout(
        title='Bollinger Bands for {}'.format(ticker_value),
        yaxis_title='Stock Price (USD per Share)',
        xaxis_title='Date',
        paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white"
    )
     
    plot_div_band = plot(fig_bb, auto_open=False, output_type='div')'''

    # Calculate daily returns
    df3['Returns'] = df3['Adj Close'].pct_change() * 100  # You can adjust the percentage format as needed

    fig_returns = go.Figure()
    fig_returns.add_trace(go.Line(x=df3.index, y=df3['Returns'], name='Daily Returns', line=dict(color='blue')))
    fig_returns.update_xaxes(
        rangeslider_visible=True
        )

    fig_returns.update_layout(
        title='{} Daily Returns'.format(ticker_value),
        yaxis_title='Daily Returns (%)',
        xaxis_title='Date'
    )
    fig_returns.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
    plot_div_returns = plot(fig_returns, auto_open=False, output_type='div')

    #scatter plot
    fig_sc = go.Figure()
    fig_sc.add_trace(go.Scatter(x=df3.index, y=df3['Adj Close'], name='Adj Close', line=dict(color='blue')))
    fig_sc.update_xaxes(
    rangeslider_visible=True
   )

    fig_sc.update_layout(
        title='{} Adjusted Close Price '.format(ticker_value),
        yaxis_title='Adj Close Price (USD per Share)',
        xaxis_title='Date'
    )
    fig_sc.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
    plot_div_sc = plot(fig_sc, auto_open=False, output_type='div')
    ...
    # Create a Plotly figure for the moving average chart
    fig_ma = go.Figure()
    fig_ma.add_trace(go.Scatter(x=df2.index, y=df2['Close'], name='Closing Price'))
    fig_ma.add_trace(go.Scatter(x=df2.index, y=df2['SMA_50'], name='50-Day SMA'))
    fig_ma.add_trace(go.Scatter(x=df2.index, y=df2['SMA_200'], name='200-Day SMA'))

    # Customize the chart layout
    fig_ma.update_layout(
        title='Moving Averages for {}'.format(ticker_value),
        yaxis_title='Stock Price (USD per Share)',
        xaxis_title='Date',
        paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white"
    )
    ...
    plot_div_ma = plot(fig_ma, auto_open=False, output_type='div')
    ...
    fig_volume.add_trace(go.Bar(x=df1.index, y=df1['Volume'], name='Volume'))
    ...
    plot_div_volume = plot(fig_volume, auto_open=False, output_type='div')
    
    
    #open low chart
    fig_open_low = go.Figure()

    fig_open_low.add_trace(go.Scatter(x=df.index, y=df['Open'], mode='lines', name='Open Prices', line=dict(color='blue')))
    fig_open_low.add_trace(go.Scatter(x=df.index, y=df['Low'], mode='lines', name='Low Prices', line=dict(color='red')))

    fig_open_low.update_layout(
        title='Open and Low Prices Over Time',
        xaxis_title='Date',
        yaxis_title='Prices',
        paper_bgcolor="#14151b",
        plot_bgcolor="#14151b",
    ...
    fig_open_low.update_xaxes(
    rangeslider_visible=True
    )

    plot_div_open_low = plot(fig_open_low, auto_open=False, output_type='div')
    ...
    context={ 'plot_div': plot_div,
              'ticker_value':ticker_value,
              'plot_div_sc': plot_div_sc,
              'plot_div_volume': plot_div_volume,
              'plot_div_open_low':plot_div_open_low,
              'plot_div_returns':plot_div_returns,
              'plot_div_ma':plot_div_ma,
